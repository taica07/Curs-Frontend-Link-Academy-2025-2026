...
function validateAndNotify() {
  ...
  if (!isValidCoupon(acceptCoupons)) {
    displayCoupon.innerHTML = "Acest cupon nu este valid";
    return;
  }
  if (acceptCoupons === "SAVE10") {
    displayCoupon.innerHTML = "Cuponul dvs. oferă 10% reducere.";
  } else if (acceptCoupons === "SAVE15") {
    displayCoupon.innerHTML = "Cuponul dvs. oferă 15% reducere.";
  } else if (acceptCoupons === "FREESHIP") {
    displayCoupon.innerHTML = "Cuponul dvs. oferă livrare gratuită.";
  }
}
...
function calculateStockProduct(products) {
  let totalValue = 0;
...
console.log(calculateStockProduct(allProducts));
...
function findProductByName(list, searchName) {
  for (let item of list) {
    if (item.name.toLowerCase().trim() === searchName.toLowerCase().trim()) {
      return item.name;
    }
  }
  return null;
...
console.log(findProductByName(allProducts, "Blouse"));
